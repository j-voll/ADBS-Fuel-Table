...
void loop() {
    ...
    adjustToNegTenPitch();  //Fixed function name
    ...
    while (millis() < movementEndTime) {
        if (millis() - lastDataTime >= DATA_INTERVAL) {
            streamCSVData("Stationary4", "None");  //Fixed phase name
            lastDataTime = millis();
        }
    }
...
float readPitch() {
    ...
    //If we reach here, we either don't have enough data or it's not the right packet
    //Don't clear the buffer - wait for more data to arrive
    return -999.0;
}
...
void adjustToZeroPitch() {
    ...
    //Now that we have a valid reading, adjust to zero
    while (abs(pitch) > 0.1) {
        if (pitch > 0) {
...
void adjustToPosFivePitch() {
    ...
    //Fix the logic for positive 5 degrees
...
void adjustToNegFivePitch() {
    ...
    //Fix the logic for negative 5 degrees
...
//Read CAN data according to the specification
CANData readCANData() {
...
//Stream data in clean CSV format
void streamCSVData(const char* phase, const char* direction) {
    ...
    if (pitch != -999.0 && pitch >= -25.0 && pitch <= 25.0) {
        ...
        //Also output debug info to Serial1 if needed
        if (Serial1) {
